...
let pixelmatchFn = null;
async function getPixelmatch() {
  if (pixelmatchFn) return pixelmatchFn;
  try {
    const mod = await import('pixelmatch');
    pixelmatchFn = mod.default || mod;
    return pixelmatchFn;
  } catch (e) {
    try {
      ...
      throw new Error('pixelmatch could not be loaded: ' + e2.message);
    }
  }
}
 ...
 * @returns {Promise<object>} { isMatch, diffPixels, totalPixels, mismatchPercentage, diffPath }
 */
